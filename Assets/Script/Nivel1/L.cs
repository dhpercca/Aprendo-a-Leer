...
{
    [SerializeField] private Transform lPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - lPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - lPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(lPlace.position.x, lPlace.position.y);
