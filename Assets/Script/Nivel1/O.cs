...
{
    [SerializeField] private Transform oPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - oPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - oPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(oPlace.position.x, oPlace.position.y);
