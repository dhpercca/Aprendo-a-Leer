...
{
    [SerializeField] private Transform pPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - pPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - pPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(pPlace.position.x, pPlace.position.y);
