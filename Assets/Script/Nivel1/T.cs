...
{
    [SerializeField] private Transform tPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - tPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - tPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(tPlace.position.x, tPlace.position.y);
