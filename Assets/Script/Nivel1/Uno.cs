...
{
    [SerializeField] private Transform unoPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - unoPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - unoPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(unoPlace.position.x, unoPlace.position.y);
