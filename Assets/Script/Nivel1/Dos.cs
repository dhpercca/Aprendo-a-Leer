...
{
    [SerializeField] private Transform dosPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - dosPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - dosPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(dosPlace.position.x, dosPlace.position.y);
