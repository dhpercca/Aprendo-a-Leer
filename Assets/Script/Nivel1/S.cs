...
{
    [SerializeField] private Transform sPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - sPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - sPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(sPlace.position.x, sPlace.position.y);
