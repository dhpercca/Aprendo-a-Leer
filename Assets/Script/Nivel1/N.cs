...
{
    [SerializeField] private Transform nPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - nPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - nPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(nPlace.position.x, nPlace.position.y);
