...
{
    [SerializeField] private Transform hPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - hPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - hPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(hPlace.position.x, hPlace.position.y);
