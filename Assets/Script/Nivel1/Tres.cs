...
{
    [SerializeField] private Transform tresPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - tresPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - tresPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(tresPlace.position.x, tresPlace.position.y);
