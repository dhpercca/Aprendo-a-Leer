...
{
    [SerializeField] private Transform cuatroPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - cuatroPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - cuatroPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(cuatroPlace.position.x, cuatroPlace.position.y);
