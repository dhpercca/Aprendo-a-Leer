...
{
    [SerializeField] private Transform zPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - zPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - zPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(zPlace.position.x, zPlace.position.y);
