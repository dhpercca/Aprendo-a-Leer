...
{
    [SerializeField] private Transform rPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - rPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - rPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(rPlace.position.x, rPlace.position.y);
