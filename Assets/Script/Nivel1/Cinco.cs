...
{
    [SerializeField] private Transform cincoPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - cincoPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - cincoPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(cincoPlace.position.x, cincoPlace.position.y);
