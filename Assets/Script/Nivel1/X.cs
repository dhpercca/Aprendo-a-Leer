...
{
    [SerializeField] private Transform wPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - wPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - wPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(wPlace.position.x, wPlace.position.y);
