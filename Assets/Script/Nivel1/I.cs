...
{
    [SerializeField] private Transform iPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - iPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - iPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(iPlace.position.x, iPlace.position.y);
