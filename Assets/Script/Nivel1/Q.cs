...
{
    [SerializeField] private Transform qPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - qPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - qPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(qPlace.position.x, qPlace.position.y);
