...
{
    [SerializeField] private Transform yPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - yPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - yPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(yPlace.position.x, yPlace.position.y);
