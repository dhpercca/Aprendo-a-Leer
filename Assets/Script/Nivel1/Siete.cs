...
{
    [SerializeField] private Transform sietePlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - sietePlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - sietePlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(sietePlace.position.x, sietePlace.position.y);
