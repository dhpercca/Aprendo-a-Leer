...
{
    [SerializeField] private Transform ochoPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - ochoPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - ochoPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(ochoPlace.position.x, ochoPlace.position.y);
