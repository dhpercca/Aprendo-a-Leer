...
{
    [SerializeField] private Transform bPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - bPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - bPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(bPlace.position.x, bPlace.position.y);
