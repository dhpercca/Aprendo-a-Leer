...
{
    [SerializeField] private Transform uPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - uPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - uPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(uPlace.position.x, uPlace.position.y);
