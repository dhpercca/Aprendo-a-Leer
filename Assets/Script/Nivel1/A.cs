...
{
    [SerializeField] private Transform aPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - aPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - aPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(aPlace.position.x, aPlace.position.y);
