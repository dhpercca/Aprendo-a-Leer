...
{
    [SerializeField] private Transform ePlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - ePlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - ePlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(ePlace.position.x, ePlace.position.y);
