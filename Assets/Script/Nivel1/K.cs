...
{
    [SerializeField] private Transform kPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - kPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - kPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(kPlace.position.x, kPlace.position.y);
