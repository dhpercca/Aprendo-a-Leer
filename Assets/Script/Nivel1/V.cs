...
{
    [SerializeField] private Transform vPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - vPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - vPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(vPlace.position.x, vPlace.position.y);
