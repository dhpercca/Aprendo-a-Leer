...
{
    [SerializeField] private Transform jPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - jPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - jPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(jPlace.position.x, jPlace.position.y);
