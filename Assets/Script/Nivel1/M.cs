...
{
    [SerializeField] private Transform mPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - mPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - mPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(mPlace.position.x, mPlace.position.y);
