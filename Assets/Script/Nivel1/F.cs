...
{
    [SerializeField] private Transform fPlace;
        ...
        if(Input.touchCount > 0 && !locked)
        {
            ...
            switch(touch.phase)
            {
                ...
                case TouchPhase.Ended:
                    if(Mathf.Abs(position1.x - fPlace.position.x) <= 0.5f && 
                       Mathf.Abs(position1.y - fPlace.position.y) <= 0.5f)
                    {
                        transform.position = new Vector2(fPlace.position.x, fPlace.position.y);
